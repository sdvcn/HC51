...
interrupt void _Timer0_isr() _at_ STC_ISR_TIMER0
{
	#if SYSTEM_TIMER == 0
		++_SystemTick;
	#else
...
interrupt void _Timer1_isr() _at_ STC_ISR_TIMER1
{
	#if SYSTEM_TIMER == 1
		++_SystemTick;
	#else
...
interrupt void _Timer2_isr() _at_ STC_ISR_TIMER2
{
	#if SYSTEM_TIMER == 2
		++_SystemTick;
	#else
...
interrupt void _Timer3_isr() _at_ STC_ISR_TIMER3
{
	#if SYSTEM_TIMER == 3
		++_SystemTick;
	#else
...
void Timer_ClearCallBack(unsigned char sel)
{
	pCallBackISR* p;
	switch (sel & 0x07)
	{
		case 0x00:p = _t0isr;break;
		case 0x01:p = _t1isr;break;
		case 0x02:p = _t2isr;break;
		case 0x03:p = _t3isr;break;
		case 0x04:p = _t4isr;break;
		default:return;
	}
	for(unsigned char i=0;i<TIMER_STACK;i++)
	{
		p[i] = (pCallBackISR)0x00;
	}
}

void Timer_PushCallBack(unsigned char sel,pCallBackISR cb)
{
	pCallBackISR* p;
	switch (sel & 0x07)
	{
